...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const contextMenuDownload = document.querySelector(
    ".js-ContextMenu-download"
  );
  ...
  links.forEach(link => {
    link.addEventListener("contextmenu", function(e) {
      ...
      contextMenuDownload.setAttribute(
        "href",
        `${window.location.pathname ? "/" : ""}download/${this.getAttribute(
          "href"
        )}`
      );
      contextMenuCopy.setAttribute(
        "value",
        `${window.location.origin}${this.getAttribute("href")}`
      );
